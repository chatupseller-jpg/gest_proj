...
async function loadSubtasksForModal(taskId) {
  const { data, error } = await supabase.from('subtasks').select('*').eq('task_id', taskId).order('position');
  ...
  list.querySelectorAll('.subtask-item').forEach(item => {
    ...
    item.addEventListener('drop', async (e) => {
      ...
      await Promise.all(arr.map((s, idx) =>
        supabase.from('subtasks').update({ position: idx }).eq('id', s.id)
      ));
      render();
    });
  });
...
async function toggleSubtask(subtaskId, checked, task) {
  await supabase.from('subtasks').update({ is_done: checked }).eq('id', subtaskId);
...
async function maybeRecalcProgress(taskId) {
  ...
  await supabase.from('tasks').update({ progress_percentage: pct }).eq('id', taskId);
  await loadTasks();
}
...
function wireDailyModal(m) {
  document.getElementById('save-daily-btn').addEventListener('click', async () => {
    ...
    if (m.daily.id) {
      await supabase.from('daily_tasks').update(payload).eq('id', m.daily.id);
    } else {
  ...
  if (delBtn) delBtn.addEventListener('click', async () => {
    ...
    await supabase.from('daily_tasks').delete().eq('id', m.daily.id);
    await loadDaily();
    closeModal();
    render();
  });
...
async function toggleDailyLog(dailyTaskId) {
  ...
  if (existing) {
    ...
    await supabase.from('daily_task_logs').update({ completed, completed_at: completed ? new Date().toISOString() : null }).eq('id', existing.id);
    existing.completed = completed;
  } else {
    const { data, error } = await supabase.from('daily_task_logs')
...
async function saveSettings() {
  await supabase.from('user_settings').upsert({ user_id: state.user.id, ...state.settings });
  applyTheme();
}
